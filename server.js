...
// Fix for MongoDB Atlas DNS resolution issues (local dev only, not needed on Vercel)
...
app.use((req, res, next) => {
  if (req.method === 'GET' && req.path.startsWith('/api')) {
    ...
    if (req.path.match(/^\/api\/products(\/paginated|\/metadata|\/[a-zA-Z0-9_-]+)?$/)) {
      // max-age=60: browser caches for 60 seconds
      // s-maxage=300: Vercel edge caches for 5 minutes
      // stale-while-revalidate=86400: Vercel serves stale data while fetching fresh in background for 24h
      res.set('Cache-Control', 'public, max-age=60, s-maxage=300, stale-while-revalidate=86400');
    } else {
      // Default: Do not cache user-specific or sensitive API routes
...
const sendErrorResponse = (res, error, defaultMessage = "An internal server error occurred") => {
  // Prevent Edge CDN from disastrously caching Server Errors due to aggressive global routing middleware
...
// One-time flag to ensure indexes are created exactly once per cold start
...
app.use(async (req, res, next) => {
  ...
  // Ensure indexes once per cold start (covers both local dev and Vercel serverless)
  if (success && !indexesEnsured) {
    indexesEnsured = true;
    Product.collection.createIndex({ id: 1 }, { background: true }).catch(() => { });
    Product.collection.createIndex({ isActive: 1, id: 1 }, { background: true }).catch(() => { });
    Product.collection.createIndex({ category: 1, id: 1 }, { background: true }).catch(() => { });
    Product.collection.createIndex({ createdAt: -1 }, { background: true }).catch(() => { });
  }
...
// For Razorpay webhooks, raw body capture is handled gracefully above
...
const calculateOrderTotal = async (cartItems, userId, reqBodyAddress = null, courierId = null, paymentMethod = "PREPAID", deliveryMethod = "STANDARD") => {
  ...
  const products = await Product.find({ id: { $in: productIds } }).select('id name price image category weightKg dimensions').lean();
...
app.post("/api/products/search-image", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    // 2. Initialize Gemini (No pre-fetching the massive catalog!)
    ...
    // 3. AI Extraction Step: Prompt Gemini to extract visual metadata only
    const prompt = `You are an expert industrial engineering AI specialized in bearings and oil seals.
Your task is to identify the bearing or seal in the provided image and extract search labels so a database can find it.
...
Respond strictly in valid JSON format mapping EXACTLY to this schema:
{
  ...
  "searchKeywords": [
    "Array of 2-5 highly relevant strings observed like engraved numbers (6204), suffixes (2RS), or types (bearing)"
  ]
}`;
    ...
    try {
      let aiData = JSON.parse(cleanedText);

      let dbMatches = [];
      if (aiData.searchKeywords && aiData.searchKeywords.length > 0) {
        const regexConditions = aiData.searchKeywords.map(kw => new RegExp(kw.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'i'));

        dbMatches = await Product.find({
          isActive: true,
          $or: [
            { sku: { $in: regexConditions } },
            { name: { $in: regexConditions } },
            { category: { $in: regexConditions } },
            { keywords: { $in: regexConditions } }
          ]
        })
          .select('id sku name category image price')
          .limit(6)
          .lean();
      }

      const mappedMatches = dbMatches.map((m, idx) => ({
        sku: m.sku,
        confidence: idx === 0 ? 95 : (85 - (idx * 5)),
        reason: `The extracted identifiers from the image matched standard product strings for: ${m.name}`
      }));

      res.json({
        detectedType: aiData.detectedType || "Unknown Part",
...
app.get("/api/products/autocomplete", async (req, res) => {
  try {
    ...
    const getRelevanceScore = (p, q) => {
      ...
      // Category & Subcategory matches (Extremely high priority for exact matches)
      ...
      // SKU matches (Priority for industrial ordering)
      if (sku === qLower) {
        score += 1500;
...
app.get("/api/products/metadata", async (req, res) => {
  try {
    const cached = cacheService.get("meta_filters");
    if (cached) return res.json(cached);
    const brands = await Product.distinct("brand", { isActive: true });
    const subcats = await Product.distinct("subcategory", { isActive: true });
    const categories = await Product.distinct("category", { isActive: true });

    // Aggregate subcategory details for rich UI cards
    const subcatDetails = await Product.aggregate([
      { $match: { subcategory: { $ne: null, $ne: "" }, isActive: { $ne: false } } },
      {
        $group: {
          _id: "$subcategory",
          count: { $sum: 1 },
      ...
      },
      {
        $project: {
          ...
          count: 1,
          image: 1,
          category: { $ifNull: ["$category", "Industrial"] }
        }
      },
      { $sort: { name: 1 } }
    ]);

    // Aggregate category details for rich UI cards
    const catDetails = await Product.aggregate([
      { $match: { category: { $ne: null, $ne: "" }, isActive: { $ne: false } } },
      {
        $group: {
          _id: "$category",
          count: { $sum: 1 },
    ...
    const result = {
      ...
      subcategoryDetails: subcatDetails,
      categoryDetails: catDetails
    };

    cacheService.set("meta_filters", result);
...
app.get("/api/products/paginated", async (req, res) => {
  try {
    ...
    const skipCount = req.query.skipCount === 'true'; // Home page doesn't need totalCount
    const minPrice = parseFloat(req.query.minPrice);
    const maxPrice = parseFloat(req.query.maxPrice);

    const cacheKey = `paginated_${page}_${limit}_${search}_${category}_${subcategory}_${brand}_${sort}_${minPrice}_${maxPrice}_${skipCount}`;
    const cached = cacheService.get(cacheKey);
    if (cached) return res.json(cached);

    // Use strict boolean true matching enabling IXSCAN composite index routing
    ...
    // Run count and find in PARALLEL instead of sequentially to halve response time
    const findPromise = Product.find(query)
      .select("id name sku category subcategory brand price image isActive isFeatured")
      .lean()
      .sort(sortOption)
      .skip((page - 1) * limit)
      .limit(limit);
    ...
    if (skipCount) {
      // Home page doesn't need count — skip entirely for faster response
      products = await findPromise;
    } else {
    ...
    cacheService.set(cacheKey, result, 1800); // Cache paginated pages for 30 minutes
    res.json(result);
  } catch (error) {
...
// Global promise cache to deduplicate simultaneous requests before cache is warm
let prodAllPromise = null;

// NEW: GET products specifically for home page without loading 1600+ items
app.get("/api/products/home", async (req, res) => {
  try {
    const cached = cacheService.get("home_products");
    if (cached) return res.json(cached);

    // 1. Fetch random 4 featured items (with fallback to random if missing)
    let featuredProducts = await Product.aggregate([
      { $match: { isFeatured: true, isActive: { $ne: false } } },
      { $sample: { size: 4 } },
      { $project: { _id: 0, id: 1, name: 1, sku: 1, category: 1, subcategory: 1, brand: 1, price: 1, image: 1, isActive: 1, isFeatured: 1 } }
    ]);

    if (featuredProducts.length < 4) {
      const excludeIds = featuredProducts.map(p => p.id);
      const backupFeatured = await Product.aggregate([
        { $match: { isActive: { $ne: false }, id: { $nin: excludeIds } } },
        { $sample: { size: 4 - featuredProducts.length } },
        { $project: { _id: 0, id: 1, name: 1, sku: 1, category: 1, subcategory: 1, brand: 1, price: 1, image: 1, isActive: 1, isFeatured: 1 } }
      ]);
      featuredProducts = [...featuredProducts, ...backupFeatured];
    }

    // 2. Fetch latest 8 novel items, excluding overlaps, using $sample to ensure a varied mix
    const featuredIds = featuredProducts.map(p => p.id);
    const newArrivals = await Product.aggregate([
      { $match: { isActive: { $ne: false }, id: { $nin: featuredIds } } },
      { $sample: { size: 8 } },
      { $project: { _id: 0, id: 1, name: 1, sku: 1, category: 1, subcategory: 1, brand: 1, price: 1, image: 1, isActive: 1, isFeatured: 1 } }
    ]);

    const result = { featuredProducts, newArrivals };
    cacheService.set("home_products", result, 1800); // 30 minutes
    res.json(result);
  } catch (error) {
    console.error("Home products fetch failed:", error);
    sendErrorResponse(res, error, "Failed to fetch home products");
  }
});

// NEW: GET brands metadata specifically for the Brands section
app.get("/api/brands/metadata", async (req, res) => {
  try {
    const cached = cacheService.get("brands_metadata");
    if (cached) return res.json(cached);

    const brandsData = await Product.aggregate([
      { $match: { brand: { $ne: null, $ne: "" }, isActive: { $ne: false } } },
      {
        $group: {
          _id: "$brand",
          count: { $sum: 1 },
    ...
    cacheService.set("brands_metadata", brandsData, 3600);
    res.json(brandsData);
  } catch (error) {
    sendErrorResponse(res, error, "Failed to fetch brands metadata");
  }
});
...
app.get("/api/products/related", async (req, res) => {
  try {
    ...
    const query = { isActive: { $nin: [false] } };
    if (excludeId) query.id = { $ne: Number(excludeId) };

    let related = [];
    if (category) {
      related = await Product.find({ ...query, category })
        .select("id name sku brand category price image")
        .limit(4).lean().sort({ id: -1 });
    }

    if (related.length < 4) {
      const remainingLimit = 4 - related.length;
      const excludeIds = related.map(p => p.id);
      if (excludeId) excludeIds.push(Number(excludeId));

      const backupQuery = { ...query, id: { $nin: excludeIds } };
      if (brand) backupQuery.brand = brand;

      const backups = await Product.find(backupQuery)
        .select("id name sku brand category price image")
        .limit(remainingLimit).lean().sort({ id: -1 });

      related = [...related, ...backups];
    }
    ...
    if (related.length < 4) {
      ...
      const finalBackups = await Product.find({ isActive: { $nin: [false] }, id: { $nin: excludeIds } })
        .select("id name sku brand category price image")
        .limit(remainingLimit).lean().sort({ _id: -1 });

      related = [...related, ...finalBackups];
    }

    res.json(related);
  } catch (error) {
    sendErrorResponse(res, error, "Failed to fetch related products");
  }
});

// GET products (Restricted to targeted batch or explicit export)
app.get("/api/products", async (req, res) => {
  try {
    // 1) Batch lookup for specific products (used by Orders / Staff dashboards)
    if (req.query.ids) {
      const idArray = req.query.ids.split(',').map(id => Number(id)).filter(id => !isNaN(id));
      if (idArray.length === 0) return res.json([]);

      const batchProducts = await Product.find({ id: { $in: idArray } })
        .select("id name sku category subcategory brand price weightKg dimensions image isActive")
        .lean();
      return res.json(batchProducts);
    }

    // 2) Explicit Admin Catalog Export
    if (req.query.export === 'true') {
      const fullCatalog = await Product.find().lean();
      return res.json(fullCatalog);
    }

    // 3) Reject global catalog pulls immediately
    return res.status(400).json({
      message: "Global catalog fetching is blocked for performance. Provide ?ids= array or use the /api/products/paginated endpoint.",
      error: "UNSUPPORTED_CATALOG_DUMP"
    });
  } catch (error) {
...
app.get("/api/products/:id", async (req, res) => {
  try {
    ...
    let product;
    if (isId) {
      product = await Product.findOne({ id: Number(param) }).lean();
      if (!product) {
...
app.post("/api/payment/create-order", auth, async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      error: error.message || (error.error ? error.error.description : String(error)),
      details: error.description || error.metadata || error.error || null
    });
  }
});
...
app.post("/api/auth/resolve-username", async (req, res) => {
  try {
    ...
    if (employee) return res.json({ email: employee.email });

    const user = await User.findOne({ username: { $regex: new RegExp(`^${username}$`, 'i') } });
    if (user && user.email) return res.json({ email: user.email });
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Construct a highly-compressed global blueprint of ALL products avoiding heavy descriptions
    const productContext = products.map(p => {
      ...
      return `- ${p.name} | SKU: ${p.sku} | Cat: ${p.category}, ${p.subcategory} | Brand: ${p.brand || 'N/A'} | ₹${p.price} | Dims: ${dimensionsStr} | Wgt: ${p.weightKg ? p.weightKg + ' kg' : 'N/A'}`;
    }).join('\n');
...
7. Add To Cart Skill: For every product recommended or mentioned in your response, ALWAYS append a functional Add to Cart link in the exact format: [Add to Cart](https://add-to-cart/SKU) right after or underneath the product description. Example: [Add to Cart](https://add-to-cart/ucp-204).`;
    ...
    // Use strictly highly-available current-gen models to prevent timeout fallback arrays causing massive latency
    const modelsToTry = [
      "gemini-1.5-flash",
      "gemini-2.0-flash",
      "gemini-flash-latest"
    ...
    let userMessage = "Sorry, I encountered an error processing your request. DIAGNOSTIC: " + (error.message || String(error));
...
if (require.main === module) {
  connectDB().then(async () => {
    // Create compound indexes for fast product queries (idempotent — safe to call every restart)
    try { await Product.collection.createIndex({ id: 1 }, { background: true }); } catch (e) { }
    try { await Product.collection.createIndex({ isActive: 1, id: 1 }, { background: true }); } catch (e) { }
    try { await Product.collection.createIndex({ category: 1, id: 1 }, { background: true }); } catch (e) { }
    try { await Product.collection.createIndex({ categoryFamilyKey: 1, id: 1 }, { background: true }); } catch (e) { }
    try { await Product.collection.createIndex({ normalizedSearch: "text" }, { background: true }); } catch (e) { }
    console.log("Indexes checked/started in background");

    app.listen(PORT, () => {
      console.log(`Server running on http://localhost:${PORT}`);

      // Cache warm-up: pre-fetch home page data so the first user request is instant
      setTimeout(async () => {
        try {
          console.log("[CACHE] Warming up product cache...");
          const homeProducts = await Product.find({ isActive: { $ne: false } })
            .select("-description -images -specifications -features -catalogue -dimensions -weightKg")
            .lean()
            .sort({ id: 1 })
            .limit(12);
          cacheService.set('paginated_1_12____default_true', { products: homeProducts, totalCount: -1, page: 1, totalPages: -1 }, 1800);

          // Metadata warming removed to allow native 5-key population on first endpoint hit

          console.log("✅ Cache warm-up complete");
        } catch (warmErr) {
          console.warn("⚠️ Cache warm-up failed (non-fatal):", warmErr.message);
        }
      }, 500);
    });
  });
...
// Initialize MongoDB for Vercel Serverless externally. 
// Vercel doesn't hit require.main === module, so connectDB must be triggered explicitly for the exported app.
